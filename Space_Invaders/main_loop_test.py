...
class enemy_entity():
    ...
    def __init__(self, state, enemyX_Change, enemyIMG, enemyX, enemyY):
        ...
        self.enemyIMG = pg.image.load(enemyIMG).convert_alpha()
        self.enemyX = enemyX
        self.enemyY = enemyY
    ...
    def collision_detection(self, entityX, entityY):
        ...
        distance = m.sqrt((m.pow((self.enemyX+27) - entityX, 2)) + (m.pow((self.enemyY+32) - entityY, 2)))  # Algabraic distance between two points
...
class  laser_entity():
    ...
    def collision_detection(self, entityX, entityY):
        ...
        distance = m.sqrt((m.pow((self.laserX) - entityX, 2)) + (m.pow((self.laserY) - entityY, 2)))  # Algabraic distance between two points
...
# Laser entity
player_laser = laser_entity(False, "Images/bullet.png", 0, 1000, -2)

# Enemy Laser Entity
enemy_laser = laser_entity(False, "Images/enemy_bullet.png", 0, 1000, 1)
...
def enemy_death():
    ...
    score += 5
    score_display.font_text = str("Score: " + str(score))
...
while running:
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            ...
            if event.key == pg.K_SPACE:
                if not player_laser.laser_state:
    ...
    # Firing Enemy Laser
    randint = r.randint(0, len(aliens)-1)

    if not enemy_laser.laser_state:

        # Validating alien state
        while aliens[randint].state == False:
        ...
        if not game_over:
            enemy_laser.laserX = aliens[randint].enemyX + 16
            enemy_laser.laserY = aliens[randint].enemyY + 32
            enemy_laser.fire_laser()

    else:
        enemy_laser.fire_laser()


    death = enemy_laser.collision_detection(player.playerX + 31, player.playerY + 31)
